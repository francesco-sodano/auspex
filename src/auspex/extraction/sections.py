...
_IGNORED_TAGS = frozenset({"script", "style", "head", "ix:header"})
...
def document_sections(document: Document, raw_text: str = "") -> list[Section]:
    if document.document_type is DocumentType.NEWS:
        text = "\n".join(part for part in (document.title, document.content_excerpt) if part)
        text = filing_html_to_text(text).strip()
        return [Section(item="news_article", text=text)] if text else []
    if document.form_type in WHOLE_DOCUMENT_FORMS:
        text = filing_html_to_text(raw_text).strip()
        return [Section(item="full_document", text=text)] if text else []
    return target_sections(document.form_type or document.document_type.value, raw_text)


def is_extraction_document(document: Document) -> bool:
    form = document.form_type or document.document_type.value
    return document.document_type is DocumentType.NEWS or form in SECTION_PATTERNS or form in WHOLE_DOCUMENT_FORMS
...
def target_sections(form_type: str, text: str) -> list[Section]:
    ...
    # A heading's own subtitle (or a subsection inside MD&A) must not terminate
    # the parent section. Only numbered Items bound numbered filings.
    boundary_positions = sorted(
        set(generic_item_positions)
        | ({p for p, _ in all_headings} if form_type == "S-1" else set())
        | {len(text)}
    )

    candidates: dict[str, list[tuple[int, int, Section]]] = {}
    for start, item in all_headings:
        ...
        candidates.setdefault(item, []).append((start, end, section))
    ...
    selected = [
        max(item_candidates, key=lambda candidate: len(candidate[2].text))
        for item_candidates in candidates.values()
    ]
    mda_ranges = [(start, end) for start, end, section in selected if section.item == "mda"]
    return [
        section
        for start, _end, section in sorted(selected, key=lambda candidate: candidate[0])
        if not (
            section.item == "results_of_operations"
            and any(mda_start <= start < mda_end for mda_start, mda_end in mda_ranges)
        )
    ]
