...
class LegacyEngineResetTests(unittest.TestCase):
    def test_transient_delete_retries_and_post_success_404_converges(self):
        calls = []

        class MissingAfterDelete(Exception):
            status_code = 404
        ...
        def operation():
            ...
            raise MissingAfterDelete("already deleted")
    ...
    def test_deterministic_delete_error_is_not_retried(self):
        calls = []
        ...
        with self.assertRaisesRegex(ValueError, "partition key"):
            _retry_operation(operation, sleeper=lambda delay: None)
    ...
    def test_plan_preserves_only_owner_identity_and_portfolio_ledger(self):
        plan = build_reset_plan(
            ...
            fabric_items=[
                ...
                {"id": "endpoint", "displayName": "auspex_bronze", "type": "SQLEndpoint"},
