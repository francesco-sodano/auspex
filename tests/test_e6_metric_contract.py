...
class E6MetricContractTests(unittest.TestCase):
    def test_metrics_notebook_creates_metric_config_and_feature_contract(self):
        ...
        self.assertIn('spark.sql("DROP TABLE IF EXISTS fact_theme_opportunity_score")', nb)
        self.assertIn('spark.sql("DROP TABLE IF EXISTS opportunity_score_snapshot_manifest")', nb)
        self.assertIn("if date_results:\n        manifest_records.append", nb)
        self.assertIn('"fact_narrative_premium",', nb)
        self.assertIn('"narrative_premium_snapshot_manifest",', nb)
        self.assertNotIn('DeltaTable.forName(spark, "fact_narrative_premium").delete()', nb)
        self.assertNotIn('DeltaTable.forName(spark, "narrative_snapshot_manifest").delete()', nb)
        ...
        self.assertIn('spark.table("security_theme_classification")', nb)
        self.assertIn("resolved_theme_classifications", nb)
        self.assertIn("effective_theme_classifications", nb)
        self.assertIn("trs_theme_classifications", nb)
        self.assertIn('F.col("c.provenance").isin("manual", "llm")', nb)
        self.assertIn('F.to_date(F.col("c.updated_at")) <= F.col("d.as_of")', nb)
        self.assertIn('F.col("provenance") == F.lit("manual")', nb)
        self.assertIn('F.col("c.classification_provenance").alias("classification_provenance")', nb)
        self.assertIn('F.col("m.membership_weight").alias("theme_proxy_weight")', nb)
        self.assertIn('F.col("b.broad_market_weight").alias("broad_market_weight")', nb)
        self.assertIn("institutional_holder_count_change_qoq", nb)
        self.assertIn("holder_counts_by_period", nb)
        self.assertIn("classification_source_freshness", nb)
        ...
        self.assertIn("cohort_leg_diagnostics", nb)
        self.assertIn("score_movement_attribution", nb)
        self.assertIn('saveAsTable("opportunity_leg_diagnostics")', nb)
        self.assertIn('saveAsTable("opportunity_score_movement")', nb)
    ...
    def test_metrics_notebook_uses_pit_filters_and_validation(self):
        ...
        for field in [
            ...
            self.assertNotIn(f'.withColumn("{field}", F.coalesce', nb)
            self.assertIn(f'.withColumn("{field}", F.col("{field}").cast', nb)
    ...
    def test_metrics_warehouse_sql_defines_required_views(self):
        ...
        financing = _read(METRICS / "20_financing_risk.sql")
        diagnostics = _read(METRICS / "21_opportunity_diagnostics.sql")
        ...
        self.assertIn("opportunity_v1", score)
        self.assertIn("balanced_v1", score)
        self.assertIn("opportunity_score_raw", score)
        self.assertIn("classification_provenance", score)
        self.assertIn("DROP VIEW IF EXISTS dbo.v_rebalance_inputs", legs)
        self.assertIn("narrative_premium", score)
        self.assertNotIn("narrative_premium_contribution", score)
        self.assertIn("dbo.fact_financing_risk", financing)
        self.assertIn("diluted_share_growth_yoy", financing)
        self.assertIn("cash_runway_years", financing)
        self.assertIn("days_since_shelf_filing", financing)
        self.assertIn("dbo.opportunity_leg_diagnostics", diagnostics)
        self.assertIn("pc1_variance_share", diagnostics)
        self.assertIn("dbo.opportunity_score_movement", diagnostics)
        self.assertIn("own_composite_effect", diagnostics)
