...
class E8ContractTests(unittest.TestCase):
    ...
    def test_sec_and_contract_notebooks_output_required_facts(self):
        ...
        self.assertIn("spark.read.text(paths)", sec_nb)
        self.assertNotIn("spark.read.json(paths)", sec_nb)
        self.assertIn('.withColumn("envelope", F.from_json("raw_json", envelope_schema))', sec_nb)
        ...
        self.assertIn("ET.fromstring(information_text)", sec_nb)
        self.assertIn('_xml_local_name(element.tag) == "infotable"', sec_nb)
