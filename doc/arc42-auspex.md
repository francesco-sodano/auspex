...
- *Current implementation:* the E6a baseline is implemented for the current E5 fact set. `nb_04_metrics` seeds `metric_weights`, computes PIT-safe market momentum/risk/risk-adjusted context from `fact_market_daily`, computes Form 4 insider smart-money metrics from `fact_insider_txn`, materializes `security_daily_features`, and publishes Lakehouse SQL endpoint projections named `v_market_momentum`, `v_market_risk`, `v_risk_adjusted`, `v_smart_money`, `v_opportunity_legs`, `v_opportunity_score`, and `v_security_daily_features` as Delta tables. The Warehouse SQL artifacts define the same `v_*` contract as true SQL views when promoted to Warehouse. Missing E8/E14 legs are surfaced in `stale_sources_json`; `opportunity_score` stays NULL with `score_status='INCOMPLETE_E6A_WAITING_E8_E14'` until E8, E14, and E6b are wired.
- *Validated in Fabric (dev, 2026-06-27):* `nb_04_metrics` converged with `security_daily_features=801`, `v_security_daily_features=801`, 0 missing/future PIT rows, 0 duplicate feature rows, 0 invalid scores, and active E6a weight sum `1.000000`. The Lakehouse SQL endpoint can query `v_security_daily_features` after the notebook materializes the `v_*` projections as Delta tables.
- *Artifacts:* current E6a baseline: `fabric/notebooks/nb_04_metrics.py`, `fabric/warehouse/metrics/{04_base_metrics.sql,12b_opportunity_legs.sql,13_opportunity_score.sql}`, `v_security_daily_features`. Future E7/E8/E14 extensions add `08_thesis.sql`, `nb_news_to_graph`, `engine/thesis.py`, and the populated six-leg score.
- *Contract:* current E6a baseline in: E5 gold facts → out: base metric views, provisional non-brake leg-source views, and stable `v_security_daily_features`. Full E6a extension in: gold facts + thesis inputs. E6b in: gold facts + thesis graph + E14 valuation-brake leg → out: final `opportunity_score`, six leg contributions, `v_security_daily_features`.
