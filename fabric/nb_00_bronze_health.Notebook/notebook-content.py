...
revision_capable_source_ids = {"prices_eod"}
if not source_ids:
...
contract_award_id = F.coalesce(
    F.get_json_object("raw_json", "$.record.award_id"),
...
raw = (
    ...
    .withColumn(
        ...
        F.when(F.col("folder_source").isin("sec_13f", "sec_13dg", "sec_8k", "sec_s1", "contracts"), F.lit(2))
        .otherwise(F.lit(expected_schema_version)),
    )
    .withColumn(
        "schema_mismatch",
        F.when(
            F.col("folder_source").isin("sec_13f", "sec_13dg", "sec_8k", "sec_s1", "contracts"),
            ~F.col("schema_version").isin(1, 2),
        ).otherwise(F.col("schema_version") != F.col("expected_source_schema_version")),
    )
...
cross_batch_key_groups = (
    ...
    .groupBy("folder_source", "natural_key")
    .agg(
        F.count("*").alias("row_count"),
...
cross_batch_metrics = (
    ...
    .groupBy(F.col("folder_source").alias("source_id"))
    .agg(
        F.count("*").alias("cross_batch_duplicate_keys"),
        F.sum(F.col("row_count") - 1).alias("cross_batch_duplicate_rows"),
        F.sum(
            F.when(F.col("payload_variants") == 1, 1).otherwise(0)
        ).alias("cross_batch_exact_duplicate_keys"),
        F.sum(
            F.when(F.col("payload_variants") > 1, 1).otherwise(0)
        ).alias("cross_batch_conflicting_keys"),
        F.sum(
            F.when(F.col("payload_variants") > 1, F.col("row_count") - 1).otherwise(0)
        ).alias("cross_batch_conflicting_rows"),
    )
)
...
source_rows = [
    (
        source,
        source in required_source_ids,
        source in revision_capable_source_ids,
    )
    for source in source_ids
]
source_frame = spark.createDataFrame(
    source_rows,
    "source_id STRING, required BOOLEAN, revision_capable BOOLEAN",
)
...
numeric_columns = [
    ...
    "cross_batch_duplicate_keys",
    "cross_batch_duplicate_rows",
    "cross_batch_exact_duplicate_keys",
    "cross_batch_conflicting_keys",
    "cross_batch_conflicting_rows",
...
health = (
    ...
    .join(cross_batch_metrics, "source_id", "left")
    ...
    .withColumn(
        "unhandled_cross_batch_conflicting_keys",
        F.when(F.col("revision_capable"), F.lit(0))
        .otherwise(F.col("cross_batch_conflicting_keys")),
    )
    .withColumn(
...
cross_batch_conflict_samples = (
    ...
    .orderBy(F.desc("payload_variants"), F.desc("batch_count"))
    .limit(100)
)
if not cross_batch_conflict_samples.isEmpty():
    display(cross_batch_conflict_samples)

cross_batch_conflict_record_samples = (
    raw.alias("records")
    .join(
        F.broadcast(
            cross_batch_conflict_samples.select("source_id", "natural_key")
        ).alias("conflicts"),
        (F.col("records.folder_source") == F.col("conflicts.source_id"))
        & (F.col("records.natural_key") == F.col("conflicts.natural_key")),
        "inner",
    )
    .select(
        F.col("conflicts.source_id").alias("source_id"),
        F.col("records.natural_key").alias("natural_key"),
        F.col("records.batch_id").alias("batch_id"),
        F.col("records.ingest_ts_raw").alias("ingest_ts"),
        F.sha2(F.col("records.record_json"), 256).alias("record_sha256"),
        F.col("records.record_json").alias("record_json"),
    )
    .orderBy("source_id", "natural_key", "ingest_ts", "batch_id")
    .limit(200)
)
cross_batch_conflict_record_rows = [
    json.loads(row) for row in cross_batch_conflict_record_samples.toJSON().collect()
]
if cross_batch_conflict_record_rows:
    display(cross_batch_conflict_record_samples)

health_rows = [json.loads(row) for row in health.toJSON().collect()]
failed_health_rows = [row for row in health_rows if row["gate_status"] == "FAIL"]
failed_sources = [row["source_id"] for row in failed_health_rows]
health_summary = {
    "from_date": from_date,
    "to_date": to_date,
    "gate_status": "FAILED" if failed_sources else "PASSED",
    "failed_sources": failed_sources,
    "sources": health_rows,
    "cross_batch_conflict_records": cross_batch_conflict_record_rows,
}
health_summary_json = json.dumps(health_summary, sort_keys=True)
failure_summary_json = json.dumps(
    {
        "from_date": from_date,
        "to_date": to_date,
        "failed_sources": failed_sources,
        "sources": failed_health_rows,
        "cross_batch_conflict_records": cross_batch_conflict_record_rows,
    },
    sort_keys=True,
)
print(f"BRONZE HEALTH SUMMARY: {health_summary_json}", flush=True)
health.unpersist()
duplicate_key_groups.unpersist()
cross_batch_key_groups.unpersist()
raw.unpersist()
window_files.unpersist()

if failed_sources:
    raise RuntimeError(f"BRONZE HEALTH FAILED: {failure_summary_json}")
...
mssparkutils.notebook.exit(health_summary_json)
