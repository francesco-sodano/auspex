...
max_retry_rows = int(_widget("max_retry_rows", "0"))
sample_limit = int(_widget("sample_limit", "100"))
if max_retry_rows < 0:
    ...
    SELECT
        *,
        CASE
            ...
            WHEN reason IN ('NO_NONDERIVATIVE_TXNS', 'INVALID_DATE') THEN 'ACCEPTED'
...
retry_rows = triage.filter(F.col("triage_status") == "RETRY").count()
retry_accessions = (
    triage.filter(F.col("triage_status") == "RETRY")
    .select("raw_identifier")
    .distinct()
    .count()
)
review_rows = triage.filter(F.col("triage_status") == "REVIEW").count()
review_accessions = (
    triage.filter(F.col("triage_status") == "REVIEW")
    .select("raw_identifier")
    .distinct()
    .count()
)
accepted_rows = triage.filter(F.col("triage_status") == "ACCEPTED").count()
resolved_rows = triage.filter(F.col("triage_status") == "RESOLVED").count()

print(
    f"Form 4 quarantine: resolved_rows={resolved_rows}, accepted_rows={accepted_rows}, "
    f"retry_rows={retry_rows}, retry_accessions={retry_accessions}, "
    f"review_rows={review_rows}, review_accessions={review_accessions}, "
    f"duplicate_keys={duplicate_natural_keys}"
)
if duplicate_natural_keys:
    raise RuntimeError(f"Quarantine natural_key duplicates found: {duplicate_natural_keys}")
if retry_accessions > max_retry_rows:
    raise RuntimeError(
        f"Retryable Form 4 accessions ({retry_accessions}) exceed threshold ({max_retry_rows}); "
