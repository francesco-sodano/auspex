...
# --- Fetch SEC company_tickers_exchange.json (CIK/ticker/name/exchange) ---
resp = requests.get(
...
raw = resp.json()
field_indexes = {name: index for index, name in enumerate(raw["fields"])}
allowed_exchanges = {"NASDAQ", "NYSE", "CBOE"}

rows = [
    (
        str(int(values[field_indexes["cik"]])),
        str(values[field_indexes["ticker"]]).upper(),
        str(values[field_indexes["name"]]),
        str(values[field_indexes["exchange"]]).upper(),
    )
    for values in raw["data"]
    if values[field_indexes["cik"]]
    and values[field_indexes["ticker"]]
    and values[field_indexes["name"]]
    and str(values[field_indexes["exchange"]]).upper() in allowed_exchanges
]
print(f"Fetched {len(rows)} supported US-listed tickers from SEC")
...
spark.sql("""
    ...
    USING DELTA
""")
_ensure_columns("security_master", {"exchange": "exchange STRING"})
...
(
    ...
    .whenMatchedUpdate(
        condition="NOT (t.company_name <=> s.company_name) OR NOT (t.exchange <=> s.exchange)",
        set={
...
security_seed = (
    source_df.select("cik", "ticker", "company_name", "exchange")
    .withColumn(
        "security_sk",
        F.pmod(
